...
geometry_msgs::Pose feedbackData, _Start, mapStart;
...
void CalXY(double x,double y,double theta,double &ans_x,double &ans_y){
	double r = sqrt(x*x+y*y);//r为距原点距离

	theta = - theta;

	double theta_absolute = asin(y/r);//绝对坐标系下的角度
	if(x<0){theta_absolute = pi - theta_absolute;}

	double theta_relative = theta_relative - theta;//相对坐标系下的角度
	if(theta_relative <= -pi){
		theta_relative += 2*pi;
	}

	ans_x = r * cos(theta_relative);
	ans_y = r * sin(theta_relative);
}
...
void updata_feedbackData(const tf::tfMessage::ConstPtr& data){
    ...
    if (data->transforms[0].header.frame_id == "map_map" && data->transforms[0].child_frame_id == "base_link" && !mapKey){
    //if (data->transforms[0].header.frame_id == "odom" && data->transforms[0].child_frame_id == "base_link" && !mapKey){
        //ROS_INFO("updata_feedbackData");
        double z[3];
        tf::Quaternion qq(data->transforms[0].transform.rotation.x, data->transforms[0].transform.rotation.y, data->transforms[0].transform.rotation.z, data->transforms[0].transform.rotation.w);
        tf::Matrix3x3 m(qq);
        m.getRPY(z[0], z[1], z[2]);
        double theta, disx, disy;
        disx = data->transforms[0].transform.translation.x;
        disy = data->transforms[0].transform.translation.y;
        if (fabs(disx) < 0.01) {
            if (disy < 0) {
                theta = 0 - pi / 2;
            } else {
            ...
            theta = atan(disy / disx);
        } else {
            if (disy > 0) {
                theta = atan(disy / disx) + pi;
            } else {
                theta = atan(disy / disx) - pi;
            }
        }
        double dis = sqrt(disx * disx + disy * disy);
        feedbackData.position.x = double(dis * cos(theta + mapEuler) + mapStart.position.x);
        feedbackData.position.y = double(dis * sin(theta + mapEuler) + mapStart.position.y);
        qq.setRPY(0, 0, mapEuler + theta + z[2]);
        feedbackData.orientation.x = double(qq.getX());
        feedbackData.orientation.y = double(qq.getY());
        feedbackData.orientation.z = double(qq.getZ());
        feedbackData.orientation.w = double(qq.getW());
        ...
        feedbackKey = false;
    } else if (data->transforms[0].header.frame_id == "map" && data->transforms[0].child_frame_id == "map_map" && !mapKey) {
        mapStart.position.x = data->transforms[0].transform.translation.x;
        mapStart.position.y = data->transforms[0].transform.translation.y;
        _Start.orientation = data->transforms[0].transform.rotation;
        double z[3];
